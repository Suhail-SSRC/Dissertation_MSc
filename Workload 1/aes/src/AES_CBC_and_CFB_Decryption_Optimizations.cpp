...
unsigned char *AES::DecryptCBC(const unsigned char in[], unsigned int inLen,
  ...
  unsigned int numBlocks = inLen / blockBytesLen;

  unsigned char *out = new unsigned char[inLen];  // Create one large array
  unsigned char *roundKeys = new unsigned char[4 * Nb * (Nr + 1)];

  KeyExpansion(key, roundKeys);

  // Initialize cipher blocks and perform decryption
  #pragma omp parallel for
  for (unsigned int i = 0; i < numBlocks; ++i) {
    DecryptBlock(in + i*blockBytesLen, out + i*blockBytesLen, roundKeys);
  }

  // Now, perform XOR operation with previous cipher block
  XorBlocks(iv, out, out, blockBytesLen);  // For the first block, use IV
  for (unsigned int i = 1; i < numBlocks; ++i) {
    XorBlocks(in + (i-1)*blockBytesLen, out + i*blockBytesLen, out + i*blockBytesLen, blockBytesLen);
  }
...
unsigned char *AES::DecryptCFB(const unsigned char in[], unsigned int inLen,
  ...
  unsigned char *out = new unsigned char[inLen]; // one large buffer for all decrypted blocks
  unsigned char *roundKeys = new unsigned char[4 * Nb * (Nr + 1)];
  unsigned char encryptedBlock[blockBytesLen];

  KeyExpansion(key, roundKeys);
  memcpy(encryptedBlock, iv, blockBytesLen);

  // Initialize cipher blocks and perform encryption
  #pragma omp parallel for
  for (unsigned int i = 0; i < numBlocks; ++i) {
    EncryptBlock(encryptedBlock, encryptedBlock, roundKeys);
    XorBlocks(in + i*blockBytesLen, encryptedBlock, out + i*blockBytesLen, blockBytesLen);
    memcpy(encryptedBlock, in + i*blockBytesLen, blockBytesLen);
  }
