...
int main() {
    ...
    vector<unsigned char> c = aes.EncryptCFB(plain, key, iv);
    vector<unsigned char> innew = aes.DecryptCFB(c, key, iv);

    for (unsigned char i: innew){
        std::cout << i << " ";
    }
    std::cout << " " << std::endl;
    for (unsigned char j: plain){
        std::cout << j << " ";
    }
    std::cout << " " << std::endl;
    if(plain == innew) {
